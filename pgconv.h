...
/* Forward + backward convolutional layer, same channel numbers */
void conv3x3_tile( 
		uint64 msb_inputs[OUT_CHANNEL_PARALLELISM][WIDTH][WIDTH], 
...
#pragma HLS PIPELINE
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				...
				msb_window_buffer[2][2] = (msb_line_buffer[1][col] = msb_inputs[OUT_CHANNEL_PARALLELISM][row][col]);
				}
...
/* Gradient convolutional layer, same batch numbers */
void convbp_tile( 
		uint64 msb_inputs[WIDTH][WIDTH], 
		const uint64 weights[OUT_CHANNEL_PARALLELISM][32][32],
		int16 msb_outputs[CHANNEL_OUT_T][WIDTH][WIDTH],
...
#pragma HLS PIPELINE
			...
			for (int channel_pt=0; channel_pt<OUT_CHANNEL_PARALLELISM; channel_pt++) {
				int16 msb_accumulation = 0;
				// Compute each output channel
				for (int k_row=0; k_row<32; k_row++) {
					for (int k_col=0; k_col<32; k_col++) {
						int row_idx_pad = row - 2 + k_row;
						int col_idx_pad = col - 2 + k_col;
						if(row_idx_pad>=0 && row_idx_pad<H_fmap_out && col_idx_pad>=0 && col_idx_pad<H_fmap_out){
							uint64 msb_a = msb_window_buffer[k_row][k_col];
