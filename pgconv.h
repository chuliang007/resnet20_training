...
using namespace std;
const int8 m1 = 6148914691236517205;
const int8 m2 = 3689348814741910323;
const int8 m4 = 1085102592571150095;


inline int8 compute_engine_64(int8 b, int8 w)
{
...
void binary_conv3x3_tile( 
		int8 msb_inputs[WIDTH][WIDTH],
		int8 weights[OUT_CHANNEL_PARALLELISM][3][3],
		int8 msb_outputs[CHANNEL_OUT_T][WIDTH][WIDTH],

		// int8 comparator[CHANNEL_OUT_T][WIDTH][WIDTH],
...
// #pragma HLS ARRAY_PARTITION variable=threshold complete dim=1
	...
	int8 msb_line_buffer[2][WIDTH] = {0};
	int8 msb_window_buffer[3][3] = {0};
...
#pragma HLS ARRAY_PARTITION variable=msb_window_buffer complete dim=0
	...
	int8 msb_partial_out_feature[OUT_CHANNEL_PARALLELISM] = {0};
					...
					for (int k_row=0; k_row<3; k_row++) {
						for (int k_col=0; k_col<3; k_col++) {
							...
							if(row_idx_pad>=0 && row_idx_pad<H_fmap_out && col_idx_pad>=0 && col_idx_pad<H_fmap_out){
								int8 msb_a = msb_window_buffer[k_row][k_col];
								int8 w = weights[channel_pt][k_row][k_col];
