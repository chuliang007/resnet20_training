...
//	float act;
//	float accum;
//	static float out_temp[CHANNEL_OUT_T][CHANNEL_IN_T][3][3] = {0};
//#pragma HLS DEPENDENCE variable=out_temp inter false
////#pragma HLS DEPENDENCE variable=output inter false
//
//#pragma HLS ARRAY_PARTITION variable=out_temp dim=1 complete
...
//
//	static float line_buffer_act[CHANNEL_IN_T][4][WIDTH] = {0};
//	static float window_buffer_act[CHANNEL_IN_T][5][5] = {0};
//#pragma HLS ARRAY_PARTITION variable=line_buffer_act complete dim=0
//#pragma HLS ARRAY_PARTITION variable=window_buffer_act complete dim=0
//
//	static float line_buffer_wt[CHANNEL_IN_T][4][WIDTH] = {0};
//	static float window_buffer_wt[CHANNEL_IN_T][5][5] = {0};
...
//									accum += wt * act;
//
////									printf("krow = %d, kcol = %d, skip_krow = %d, skip_kcol = %d, ", krow, kcol, skip_krow.to_int(), skip_kcol.to_int());
////									printf("accum = %d \n", accum.to_int());
//								}
//							}
//
...
//	float out_buf_SC[CHANNEL_OUT_T][WIDTH][WIDTH],		// out_copy, ideneity for generic_shortcut
	...
	for (int row = 0; row < H_fmap_in; row ++) {
		for (int col = 0; col < H_fmap_in; col ++) {
			for (int c = 0; c < CHANNEL_OUT_T; c ++) {
				...
				out_buf_DDR[c][row][col] = bn_wt[c]*(bn_inputs[c][row][col]-mu[c])/(std_var[c] + eps) + bn_bias[c];
			}
		}
	}
}
...
)
{
	...
	// grad of generic_bn params
	for (int row = 0; row < H_fmap_in; row ++) {
		for (int col = 0; col < H_fmap_in; col ++) {
			for (int c = 0; c < CHANNEL_OUT_T; c ++) {
				g_bn_bias[c] += error[c][row][col];
				g_bn_wt[c] += error[c][row][col] * (bn_inputs_fw[c][row][col]-mu[c])/(std_var[c]+eps);
			}
		}
	}
	...
	for (int row = 0; row < H_fmap_in; row ++) {
		for (int col = 0; col < H_fmap_in; col ++) {
			for (int c = 0; c < CHANNEL_OUT_T; c ++) {
				out_buf[c][row][col] = bn_wt[c]*error[c][row][col]/(std_var[c]+eps) - bn_wt[c]*g_bn_bias[c]/(N*(std_var[c]+eps)) - bn_wt[c]*(bn_inputs_fw[c][row][col]-mu[c])*g_bn_wt[c]/(N*(std_var[c]*std_var[c]+eps));
			}
		}
	}
...
// generic_maxpool(
// 	float max_inputs[CHANNEL_IN_T][WIDTH][WIDTH],	// in,  32x 32x32, 64x 16x16
// 	float out_buf[CHANNEL_IN_T][WIDTH][WIDTH],		// out, 32x 16x16, 64x 8x8

// 	uint1 ctrl_maxpool,	// 0 for forward and 1 for backward
// 	int c_out,
...
// )
// {
// 	float max_temp[CHANNEL_IN_T][WIDTH][WIDTH];
...
// 				for (int c = 0; c < CHANNEL_IN_T; c ++) {
// 					out_buf[c][row][col] = 0;
// 					max_temp[c][row][col] = 0;
// 				}
// 			}
// 		}
// 		// find max
// 		for (int row = 0; row < H_fmap_in; row = row+2) {
// 			for (int col = 0; col < H_fmap_in; col = col+2) {
// 				for (int c = 0; c < CHANNEL_IN_T; c ++) {
// 					for (int s = 0; s < 2; s ++) {
// 						for (int ss = 0; ss < 2; ss ++) {
// 							if (max_inputs[c][row + s][col + ss] > max_temp[c][row/2][col/2])
// 								max_temp[c][row/2][col/2] = max_inputs[c][row + s][col + ss];
// 						}
// 					}
// 				}
// 			}
...
// 			for (int col = 0; col < H_fmap_in; col ++) {
// 				for (int c = 0; c < CHANNEL_IN_T; c ++) {
// 					out_buf[c][row/2][col/2] = max_temp[c][row][col];
// 				}
// 			}
// 		}
// 	}
