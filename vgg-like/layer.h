...
#pragma HLS ARRAY_PARTITION variable=weight_rot dim=1 complete
#pragma HLS ARRAY_PARTITION variable=weight_rot dim=2 complete
#pragma HLS ARRAY_PARTITION variable=weight_rot dim=3 complete
#pragma HLS ARRAY_PARTITION variable=weight_rot dim=4 complete

#pragma HLS ARRAY_PARTITION variable=weight_tmp dim=1 complete
#pragma HLS ARRAY_PARTITION variable=weight_tmp dim=2 complete
#pragma HLS ARRAY_PARTITION variable=weight_tmp dim=3 complete
#pragma HLS ARRAY_PARTITION variable=weight_tmp dim=4 complete
...
)
{
	...
	float wt[CHANNEL_OUT_T] = {0};
	float act[CHANNEL_IN_T] = {0};
	float out_tmp[CHANNEL_OUT_T][CHANNEL_IN_T] = {0};
	float out_temp[CHANNEL_OUT_T][CHANNEL_IN_T] = {0};
...
#pragma HLS PIPELINE II=1
					...
					for (int co = 0; co < CHANNEL_OUT_T; co ++) {
						for (int cin = 0; cin < CHANNEL_IN_T; cin ++) {
							...
							for (int pkrow = 0; pkrow < 4; pkrow ++) {
								for (int pkcol = 0; pkcol < 4; pkcol ++) {
									// conv(0,0)
									if (krow >= 0 && krow < H_fmap_in && kcol >= 0 && kcol < H_fmap_in && skip == 0) {
										wt[co] = window_buffer_wt[co][pkrow][pkcol];
										act[cin] = window_buffer_act[cin][pkrow][pkcol];
									}
								}
					...
					for (int co = 0; co < CHANNEL_OUT_T; co ++) {
						for (int cin = 0; cin < CHANNEL_IN_T; cin ++) {
							if (krow >= 0 && krow < H_fmap_in && kcol >= 0 && kcol < H_fmap_in && skip == 0) {
								out_temp[co][cin] += out_tmp[co][cin];
							}
						}
					}
				}
